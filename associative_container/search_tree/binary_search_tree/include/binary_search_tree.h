...
public:
	...
	public:
		...
		explicit prefix_reverse_iterator(const prefix_iterator&) noexcept;

		explicit operator prefix_iterator() const noexcept;
	...
	public:
		...
		explicit prefix_const_reverse_iterator(const prefix_const_iterator&) noexcept;

		explicit operator prefix_const_iterator() const noexcept;
...
typename binary_search_tree<tkey, tvalue, compare, tag>::infix_iterator binary_search_tree<
	...
	if (_logger) {
		_logger->log("Deleted node", logger::severity::debug);
	}
