...
// Если длина правого поддерева - длина левого поддерева == 2 (у какой-то вершины), то это будет левое вращение
// Если длина правого поддерева правого поддерева >= длины левого поддерева правого поддерева, то малое левое, иначе левое поддерево правого > правого поддерева правового - большое левое вращение

// Если длина левого поддерева - длина правого поддерева = 2 (у какой-то вершины), то это уже правое вращение
// Если длина левого поддерева левого поддерева >= длины правое поддерева левого поддерева, то малое правое, иначе - большое правое

// Вставка/поиск/удаление O(logn), а повороты O(1)
			...
			typename binary_search_tree<tkey, tvalue, compare, AVL_TAG>::node** node_ptr)
	{
		...
		if (!node->left_subtree && !node->right_subtree) {
			...
			if (node->left_subtree) {
				child = node->left_subtree;
			} else {
				child = node->right_subtree;
			}
...
void AVL_tree<tkey, tvalue, compare>::node::recalculate_height() noexcept
{
	...
	if (this->right_subtree) {
		right_height = static_cast<node*>(this->right_subtree)->height;
	} else {
		right_height = 0;
	}
